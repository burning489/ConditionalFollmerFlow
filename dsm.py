...
cfg = importlib.import_module("config.ode_mnist").cfg
...
def get_score_fn():
    def score_fn(x, t):
        ...
        score = model(x, t*cfg.sampling.T, None)
        # scale output
...
for epoch in range(cfg.train.max_epochs):
    ...
    if (epoch + 1) % cfg.train.sampling_freq == 0:
        model.eval()
        with torch.no_grad():
            noise = sde.prior_sampling([cfg.sampling.batch_size // dist.get_world_size(), *cfg.data.shape]).to(device)
            ema.store(model.parameters())
            ema.copy_to(model.parameters())
            pred_per_gpu = sde.sampling(score_fn, noise=noise)
            pred = torch.empty([cfg.sampling.batch_size, *cfg.data.shape], device=device)
            dist.all_gather_into_tensor(pred, pred_per_gpu)
            ema.restore(model.parameters())
            if dist.get_rank() == 0:
                fig = make_grid(pred, nrow=int(math.sqrt(cfg.sampling.batch_size)))
