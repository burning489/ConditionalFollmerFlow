...
class MLP(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        x_layers = [nn.Linear(data_dim, hidden_dim), ]
        for _ in range(n_blocks):
            x_layers.append(ResidualBlock(hidden_dim))
        self.x_module = nn.Sequential(*x_layers)
        y_layers = [nn.Linear(cond_dim, hidden_dim), ]
        for _ in range(n_blocks):
            y_layers.append(ResidualBlock(hidden_dim))
        self.y_module = nn.Sequential(*y_layers)
        self.t_module = nn.Sequential(TimeEmbedding(t_emb_dim), nn.SiLU(), nn.Linear(t_emb_dim, hidden_dim))
    ...
    def forward(self, x, t, y):
        x, y, t = x.float(), y.float(), t.float().flatten()
        x_emb = self.x_module(x)
        t_emb = self.t_module(t)
        y_emb = self.y_module(y)
        h = self.out_module(x_emb + y_emb + t_emb)
        return h.float()
