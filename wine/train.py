...
def create_parser():
    ...
    defaults = dict(device=0, seed=42, output='logs/', desc="", mode="follmer",  # meta
                    data='dataset/wine.pkl', n_train=5847,  # data
                    eps=1e-3, T=0.999, N=100, # diffusion
                    sigma_min=0.1, sigma_max=100, # VE SDE specific
                    latent_dim=50,  # VAE & GAN
                    n_critic=1, gp_weight=0.05, # GAN specific
                    bsz=1000, train_steps=40000, lr=5e-4,  # training
...
if __name__ == '__main__':
    ...
    assert args.n_train <= x.shape[0]
    args.data_dim = x.shape[-1]
    args.cond_dim = y.shape[-1]

    # Dataset and Dataloader
    dataset = TensorDataset(x, y)
    bsz = min(args.bsz, args.n_train)
    loader = DataLoader(dataset, batch_size=bsz, shuffle=True, drop_last=True)
    ...
    logger.info(f'global batch size: {bsz}')
