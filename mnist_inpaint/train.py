...
from misc import add_dict_to_argparser, default_logger, create_infinite_dataloader, mask_fn
...
def create_parser():
    ...
    defaults = dict(device=0, seed=42, output='logs/', desc="", mode="follmer", num_workers=2,  # meta
                    n_train=60000, 
                    mask_mode=1, # masked percentage: 1 - 75% , 2 - 50%, 3 - 25%
                    ...
                    bsz=256, train_steps=10000, lr=1e-3, ema_decay=0.999,  # training
...
if __name__ == '__main__':
    ...
    desc = f'{args.mode}-mask{args.mask_mode}-n{args.n_train}'
    if args.desc:
        desc += f'-{args.desc}'
